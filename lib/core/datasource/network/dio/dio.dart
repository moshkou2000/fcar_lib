...
class DioNetwork extends INetwork {
  static DioNetwork? _singleton;
  factory DioNetwork({required String baseUrl, required IDatabase? database}) {
    if (_singleton == null) {
      _singleton = DioNetwork._internal(baseUrl: baseUrl);
      _singleton!._network = _DioNetwork(database: database);
    }
    return _singleton!;
  }
...
class _DioNetwork {
  ...
  static _DioNetwork? _singleton;
  factory _DioNetwork({required IDatabase? database}) {
    _singleton ??= _DioNetwork._internal(database: database);
    return _singleton!;
  }
  _DioNetwork._internal({required IDatabase? database}) {
    ...
    cacheStore = database != null ? DioCache(database: database) : null;
